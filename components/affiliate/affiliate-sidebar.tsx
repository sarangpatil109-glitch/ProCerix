...
export function AffiliateSidebar({ name, coupon }: { name: string; coupon: string }) {
    ...
    <div className="w-[240px] shrink-0 h-full bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 flex flex-col">
      ...
      <div className="h-16 flex items-center px-5 border-b border-gray-100 dark:border-gray-800 gap-2 shrink-0">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 font-mono">{coupon}</p>
        </div>
      </div>
      ...
      <nav className="flex-1 py-3 px-2 space-y-0.5 overflow-y-auto">
        {NAV.map(item => {
          ...
          return (
            ...
            >
              <item.icon className={`w-4 h-4 shrink-0 ${isActive ? "text-blue-600 dark:text-blue-400" : "text-gray-400"}`} />
              {item.title}
            </Link>
          );
        })}
